...
#axes.set_ylim([ymin,ymax])
    ...
    ax.plot(xy[0],feet*xy[1], 'g', linewidth=6, alpha=0.15) #lineup
    ax.plot(xy[0],feet*xy[1], 'w-', linewidth=1, alpha=0.45) #lineup
...
#    ax.add_patch(rect)
    ...
    if hornet_aoa in ps:    # 7.4 on speed min, 8.1 on speed, 8.8 onspeed max
        ...
        print('AOA line displayed for Harrier')
        ax.plot([0,xmax],[12.0,10.0],'g-',linewidth=1.2,alpha=0.8,linestyle='--') #g-
        ax.plot([0,xmax],[12.0,10.0],'g-',linewidth=1.2,alpha=0.8,linestyle='--') #g-
    elif skyhawk_aoa in ps: # 8.5 onspeed min, 8.75 onspeed, 9.0 onspeed max
        if maxvalue < 16.75 and minvalue > 9.75:
            print('AOA * maxvalue < 17.5 and minvalue > 12.5 *')
            maxvalue = 15.01
            minvalue = 9.74

        if maxvalue > 119.75 and minvalue > 6.75:
            print('AOA ** maxvalue > 17.5 and minvalue > 12.5 **')
            minvalue = 4.74
        #if minvalue > 5.01:
        #    minvalue = 5.01
        ...
        ax.plot([0,xmax],[13.25,13.25],'g-',linewidth=1.2,alpha=0.8,linestyle='--') #g-
        ax.plot([0,xmax],[12.25,12.25],'g-',linewidth=1.2,alpha=0.8,linestyle='--') #g-
    else:
        print('no SH pass drawn')
...
#    ax.tick_params(axis='y', colors='red')
    ...
    if unicorn in ps:
        ...
        bgpic = plt.imread('Images/LSO.png')
        ax.figure.figimage(bgpic, 150, 415, alpha=.5, zorder=1)
        print('unicorn in file name so change the grade to _OK_')
        titlestr+='_OK_'
        titlestr+=' '
        titlestr+='5.0'
        ...
        print('WOP in file name so change the grade to WOP')
        titlestr+='WOP'
        titlestr+=' '
        titlestr+='2.0'
        titlestr+='PT'
