import csv
import sys
import gspread
from oauth2client.service_account import ServiceAccountCredentials
from datetime import datetime
from private_api_keys import *
#print('Length of sys.argv:')
#print(len(sys.argv))
if len(sys.argv) == 2:
	
	line = sys.argv[1]	
	# line = "TG, (OK), 3.0 PT, F(LOLUR)X F(LOLUR)IM  (F)IC , 1-wire, groove time 22.0 seconds, (CASE I)"
	#x = csv.reader(line)
	#y = line.split(",")

	result = [x.strip() for x in line.split(',')]
	
	#now = datetime.now() # current date and time
	#result.append(now.strftime("%d/%m/%Y")) #commented out as I think I'll have the server date and time sent to the table when created
	#result.append(now.strftime("%H:%M:%S"))
#	print(list(result))
	
	scope = ['https://spreadsheets.google.com/feeds','https://www.googleapis.com/auth/drive']
	creds = ServiceAccountCredentials.from_json_keyfile_name(servCreds, scope)
	client = gspread.authorize(creds)

	sheet = client.open(sheetName).worksheet(rangeWorksheet)
	list_of_hashes = sheet.get_all_records()


	print('Inserting Range data into Google sheet...')

	row = result
#	print(row)

	index = 2
	sheet.insert_row(row, index)
else:	
	print('error - not enough arguments')